...
This repository vendors the ast-index skill under `.agents/skills/ast-index`.
Do not install the upstream Claude plugin from this repo unless the user
explicitly asks for Claude Code global integration. Prefer the repo-local
adapters and `.agents/integrations/ast-index.md`.
...
ast-index install-claude-plugin      # Global Claude Code only; do not run here unless explicitly asked
```
