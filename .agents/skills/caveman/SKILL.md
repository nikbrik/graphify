...
This repository uses caveman as repo-local instructions, not as a Claude hook
install. Do not write global Claude/Codex/IDE config and do not run upstream
`install.sh`, `install.ps1`, `node bin/install.js --all`, or `npx ... -- --all`
unless the user explicitly asks for a global install.

For Russian-language prompts, answer in Russian with terse technical prose.
Keep technical terms, code, paths, commands, URLs, identifiers, and exact errors
unchanged.
